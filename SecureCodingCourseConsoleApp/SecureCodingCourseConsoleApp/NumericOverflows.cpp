...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    //UPDATED CODE
    //Find the max value for the current variable type
    T maxValue = (std::numeric_limits<T>::max)();

    for (unsigned long int i = 0; i < steps; ++i)
    {
        //UPDATED CODE
        //Test for potential overflow occuring if we exceed the max value of the type with our addition during a given step in the iterative addition:
        if (increment > 0 && result > maxValue - increment)
        {
            //Provide user feedback regarding overflow. and return null if overflow would occur.
            std::cout << "\toverflow will occur if " << +increment << " added to current addition iteration" << std::endl;
            return NULL;
        }
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    {
        //UPDATED CODE
        //Test for potential underflow occuring if we go below the min value of the type with our subtraction during a given step in the iterative subtraction:
        if (decrement > 0 && result < minValue + decrement)
        {
            //Provide user feedback regarding overflow and return null if underflow would occur.
            std::cout << "\tunderflow will occur if " << +decrement << " subtracted from current subtration iteration" << std::endl;
            return NULL;
        }
...
void test_overflow()
{
    ...
    //CODE UPDATE
    //Only display the result if null was not returned for result due to an overflow occuring.
    if (result) 
    {
...
void test_underflow()
{
    ...
    //CODE UPDATE:  See Note below.
    //NOTE:  Output for test suggested initially it was testing for subraction with and without Overflow, when I believe it means it is testing for subtration with and without underflow underflow.  
    //Output to user adjusted for clarification.
    std::cout << "\tSubtracting Numbers Without Underflow (" << +start << ", " << +decrement << ", " << steps << ") = ";
    auto result = subtract_numbers<T>(start, decrement, steps);
    //CODE UPDATE
    //Only display the result if null was not returned for result due to an underflow occuring.
    if (result)
    {
        std::cout << +result << std::endl;
    };

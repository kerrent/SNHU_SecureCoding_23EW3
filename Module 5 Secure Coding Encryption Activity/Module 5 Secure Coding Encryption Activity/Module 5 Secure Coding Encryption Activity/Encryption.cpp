...
std::string read_file(const std::string& filename)
{
    ...
    //use line based parsing to get the information from the file.
    std::string line;
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    //We may as well get the timestamp we are going to use before opening the stream.
    std::time_t currentTime;
    std::time(&currentTime);
    struct tm timeInfo;
    localtime_s(&timeInfo, &currentTime);
    char buffer[32];
    std::strftime(buffer, 32, "%Y-%m-%d", &timeInfo);
    std::string timeString = buffer;
    
    std::ofstream saveData;
    //open the stream for saving to a file based on the filename parameter
    saveData.open(filename);
    //Make sure the file opened.
    if(!saveData) 
    {
        std::cerr << "File to save data failed to open";
    }

    //NOTE:  Because the data includes the student name, Student name will appear twice in the file, once on line 1 and again at the start of line 4 with the data.

    //output information to file
    saveData << student_name << std::endl << timeString << std::endl << key << std::endl << data << std::endl;
    //close the stream.
    saveData.close();
